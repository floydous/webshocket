...
class _TestRpcHandler(webshocket.WebSocketHandler):
    ...
    async def on_receive(self, connection: webshocket.ClientConnection, packet: webshocket.Packet):
        # This method should only be called for non-RPC packets after the refactoring
        if packet.source != PacketSource.RPC:
            await connection.send(f"Non-RPC Echo: {packet.data}")
        else:
            # If an RPC packet somehow reaches here without being dispatched,
            # it indicates an issue with the handler's on_receive logic.
            # For testing, we might want to explicitly fail or log this.
            pass
    ...
    async def non_rpc_method(self, connection: webshocket.ClientConnection):
        # This method should NOT be callable via RPC
        return "This should not be called via RPC"


@pytest_asyncio.fixture
async def rpc_server():
    server = webshocket.WebSocketServer("localhost", 5001, clientHandler=_TestRpcHandler)
    await server.start()
    yield server
    await server.close()
...
async def test_rpc_echo_method(rpc_server):
    async with webshocket.WebSocketClient("ws://localhost:5001") as client:
        ...
        assert isinstance(response_packet.rpc_data, RPCResponse)
        assert response_packet.rpc_data.error is None
        assert response_packet.rpc_data.result == "Hello RPC World"
        assert response_packet.data == "Hello RPC World"  # client.recv extracts result to data
...
async def test_rpc_add_numbers(rpc_server):
    async with webshocket.WebSocketClient("ws://localhost:5001") as client:
        ...
        assert isinstance(response_packet.rpc_data, RPCResponse)
        assert response_packet.rpc_data.error is None
        assert response_packet.rpc_data.result == 30
        assert response_packet.data == 30
...
async def test_rpc_method_alias(rpc_server):
    async with webshocket.WebSocketClient("ws://localhost:5001") as client:
        ...
        assert isinstance(response_packet.rpc_data, RPCResponse)
        assert response_packet.rpc_data.error is None
        assert response_packet.rpc_data.result == 12
...
async def test_rpc_method_not_found(rpc_server):
    async with webshocket.WebSocketClient("ws://localhost:5001") as client:
        ...
        assert response_packet.source == PacketSource.RPC
        assert isinstance(response_packet.rpc_data, RPCResponse)
        assert response_packet.rpc_data.result is None
        assert "not found" in response_packet.rpc_data.error
...
async def test_rpc_method_raises_rpc_error(rpc_server):
    async with webshocket.WebSocketClient("ws://localhost:5001") as client:
        ...
        assert isinstance(response_packet.rpc_data, RPCResponse)
        assert response_packet.rpc_data.result is None
        assert response_packet.rpc_data.error == error_msg
...
async def test_rpc_method_raises_generic_exception(rpc_server):
    async with webshocket.WebSocketClient("ws://localhost:5001") as client:
        ...
        assert isinstance(response_packet.rpc_data, RPCResponse)
        assert response_packet.rpc_data.result is None
        assert "Server error: ValueError: This is a generic value error." in response_packet.rpc_data.error
        assert "Server error: ValueError: This is a generic value error." in response_packet.data
...
async def test_on_receive_handles_non_rpc_packet(rpc_server):
    async with webshocket.WebSocketClient("ws://localhost:5001") as client:
        ...
        assert response_packet.source == PacketSource.CUSTOM  # Or UNKNOWN, depending on client.send behavior
        assert response_packet.rpc_data is None
